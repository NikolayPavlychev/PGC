...
if str(sys.argv[2]) =='train':
    ...
    dataset_train_test_target = pd.read_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target.csv',sep=';')
    ...
    classes = np.unique(y)
    features_coo = scipy.sparse.load_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr.npz')
    ...
    joblib.dump(clf,ROOT_DIR + '/models/'+prefix_name+'_lr_model.pickle')
...
if str(sys.argv[2]) == 'inference':
    ...
    dataset_train_test_target = pd.read_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target'+'_inference.csv',sep=';')
    ...
    with open(ROOT_DIR+ '/outputs/'+prefix_name+'_predicted_targets.txt', 'r') as f:
    ...
    classes = np.unique(y)
    features_coo = scipy.sparse.load_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr'+'_inference.npz')
    features_csr = features_coo.tocsr()[7:,:]
    print(features_csr.shape)
    features_csr = features_csr[ind,:]
    print(y_predicted.shape)
    ...
    clf = joblib.load(ROOT_DIR + '/models/'+prefix_name+'_lr_model.pickle')
    X = features_csr
    y = target
    X_id = y_predicted
    ...
    target_group_code_mapping = joblib.load(ROOT_DIR+ '/outputs/'+prefix_name+'_target_group_code_mapping_full.pickle')
    ...
    X_id.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_predictions'+'_inference.csv',sep=';',index=False)
...
if str(sys.argv[2]) == 'inference_production':
    ...
    dataset_train_test_target = pd.read_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target'+'_inference_production.csv',sep=';',nrows=50000)
    ...
    with open(ROOT_DIR+ '/outputs/predicted_targets.txt', 'r') as f:
        predicted_targets = f.readlines()
        f.close()
    ...
    classes = np.unique(y)
    features_coo = scipy.sparse.load_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr'+'_inference_production.npz')
    features_csr = features_coo.tocsr()[7:,:]
    print(features_csr.shape)
    features_csr = features_csr[ind,:]
    print(y_predicted.shape)
    ...
    clf = joblib.load(ROOT_DIR + '/models/lr_prod.pickle')
    X = features_csr
    y = target
    X_id = y_predicted
    ...
    target_group_code_mapping = joblib.load(ROOT_DIR+ '/outputs/target_group_code_mapping_full.pickle')
    target_group_code_mapping = dict((v, int(k)) for k, v in target_group_code_mapping.items())

    X_id['group_code_predict'] = X_id['predict'].apply(lambda x: target_group_code_mapping[x])
    X_id = X_id[['artical','group_code','group_code_predict','predict_proba']]
    X_id.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_predictions'+'_inference_productions.csv',sep=';',index=False)
