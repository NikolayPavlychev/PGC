...
print(ROOT_DIR)
sys.path.insert(0,ROOT_DIR + '/functions')
sys.path.insert(1,ROOT_DIR + '/models')
sys.path.insert(2,ROOT_DIR + '/outputs')
...
dataset = pd.read_csv(ROOT_DIR + '/' + input_data_file, dtype=object,sep=';', header=0,usecols=cols)
...
if str(sys.argv[2]) =='train':
    ...
    joblib.dump(enc, ROOT_DIR+ '/models/'+prefix_name+'_label_encoder.pickle')
    dataset['target'] = enc.transform(dataset['group_code'])

    le_name_mapping = dict(zip(enc.classes_, enc.transform(enc.classes_)))
    joblib.dump(le_name_mapping,ROOT_DIR+ '/models/'+prefix_name+'_target_group_code_mapping_full.pickle')

    brends_list = ['brend_code'+'__'+str(brend) for brend in list(dataset['brend_code'].unique())]

    with open(ROOT_DIR+ '/models/'+prefix_name+'_brends_list.txt', 'w') as f:
...
if str(sys.argv[2]) =='inference':
    ...
    enc = joblib.load(ROOT_DIR+ '/models/'+prefix_name+'_label_encoder.pickle')
    dataset['target'] = enc.transform(dataset['group_code'])

if str(sys.argv[2]) =='inference_production':

    enc = joblib.load(ROOT_DIR+ '/models/label_encoder.pickle')
    dataset['target'] = enc.transform(dataset['group_code'])
...
dataset.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_dataset_groups_preprocessing_filter.csv', sep=';', index=False)
print('process time: ',round(time.time()-time_start,2),' s')
#-----------------------------------------------------------------------------------------------------------------------
print('Tokenization')
time_start = time.time()
...
rowcount  = 0
for row in open(ROOT_DIR +'/outputs/'+prefix_name+'_dataset_groups_preprocessing_filter.csv'):
  rowcount+= 1


# header = True

output_path = ROOT_DIR + '/outputs/'+prefix_name+'_dataset_train_test_preprocessing_lemma.csv'

for k, chunk in enumerate(pd.read_csv(ROOT_DIR +'/outputs/'+prefix_name+'_dataset_groups_preprocessing_filter.csv',sep=';',chunksize=rowcount//20)):
    print('\n')
    print('chunk ',k)
    print('Lemmatization')
...
dataset_train_test = pd.read_csv(ROOT_DIR + '/outputs/'+prefix_name+'_dataset_train_test_preprocessing_lemma.csv',sep='|',dtype=object)
...
if str(sys.argv[2]) =='train':
    ...
    with open(ROOT_DIR+ '/outputs/'+prefix_name+'_tfidf_filter_indexes.txt', 'w') as f:
    ...
    scipy.sparse.save_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr.npz', features_csr, compressed=True)
    dataset_sub_features.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target.csv',sep=';',index=False)

if str(sys.argv[2]) =='inference':

    tfidf_vectorizer = joblib.load(ROOT_DIR+ '/models/'+prefix_name+'_tfidf_vectorizer.pickle')
    X_tfidf = tfidf_vectorizer.transform(dataset_train_test['desc_clear_norm'])
    X_tfidf=X_tfidf.tocsc(copy=False)

    with open(ROOT_DIR+ '/models/'+prefix_name+'_tfidf_filter_indexes.txt', 'r') as f:
    ...
    features_csr = scipy.sparse.hstack((dataset_sub_brand_ohe, X_tfidf_keywords))
    scipy.sparse.save_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr_inference.npz', features_csr, compressed=True)
    dataset_sub_features.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target_inference.csv',sep=';',index=False)

if str(sys.argv[2]) =='inference_production':
    print('Inference production preprocessing')
    tfidf_vectorizer = joblib.load(ROOT_DIR+ '/models/tfidf_vectorizer.pickle')
    X_tfidf = tfidf_vectorizer.transform(dataset_train_test['desc_clear_norm'])
    X_tfidf=X_tfidf.tocsc(copy=False)

    with open(ROOT_DIR+ '/models/tfidf_filter_indexes.txt', 'r') as f:
        index_features = f.readlines()
    ...
    enc = joblib.load(ROOT_DIR+ '/models/ohe_encoder.pickle')
   ...
   # dataset_sub_brand_ohe,cols_ohe = OhePreprocessing(dataset=dataset_sub_features['brend_code'], engine='inference_production',prefix_name=prefix_name)
    ...
    features_csr = scipy.sparse.hstack((dataset_sub_brand_ohe, X_tfidf_keywords))
    scipy.sparse.save_npz(ROOT_DIR + '/outputs/'+prefix_name+'_features_csr_inference_production.npz', features_csr, compressed=True)
    dataset_sub_features.to_csv(ROOT_DIR + '/outputs/'+prefix_name+'_id_target_inference_production.csv',sep=';',index=False)
